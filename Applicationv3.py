...
imlec = conn.cursor()
imlec.execute('''CREATE TABLE IF NOT EXISTS  SunucuListesi (IPlist text, hostnamelist text)''')
conn.commit()
...
def SSH(*args):
    ...
    if len(sunucularIP)==0:
        ...
        imlec.execute('''INSERT INTO  SunucuListesi (IPlist ) VALUES (?)''', [IP.get()])
...
#LISTING ALL IPs in Metadata DB

def foo():
    data = imlec.execute('''SELECT * FROM Sunuculistesi ''')
    IPlistesi = data.fetchall()
